export default async function handler(request, response) {
  ...
  try {
    ...
    const apiResponse = await fetch(
      `${apiBaseUrl.replace(/\/$/, '')}/password_reset/verify`,
      {
        method: 'POST',
        headers: {
    ...
    if (!contentType.includes('application/json')) {
      ...
      console.error('Password reset verify API returned a non-JSON response', {
    ...
    console.error('Password reset verify proxy failed', error)
